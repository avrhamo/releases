...
const RSA: React.FC<BaseToolProps> = (props) => {
  ...
  React.useEffect(() => {
    if (!state.mode) {
      setState({ mode: 'encrypt', showKeyManager: false });
    }
  }, [state.mode, setState]);
  ...
  const handleModeChange = (mode: 'encrypt' | 'decrypt') => {
    setState({ mode, output: '' });
  };

  const handleKeyChange = (type: 'public' | 'private', value: string) => {
    setState({ [type === 'public' ? 'publicKey' : 'privateKey']: value, output: '' });
  };
...
-----END PRIVATE KEY-----`;
  ...
  const processInput = () => {
    try {
      if (!state.input?.trim()) {
        setState({ output: 'Error: Please enter text to process' });
        return;
      }

      const requiredKey = state.mode === 'encrypt' ? state.publicKey : state.privateKey;
      if (!requiredKey?.trim()) {
        setState({ output: `Error: Please enter a ${state.mode === 'encrypt' ? 'public' : 'private'} key` });
        return;
      }

      // Mock RSA processing with clean output
      let output: string;
      
      if (state.mode === 'encrypt') {
        // Mock encryption - produce a clean base64-like encrypted string
        output = `${btoa(state.input).replace(/[+/=]/g, '')}ABC123XYZ`;
      } else {
        ...
        try {
          ...
          output = textToDecrypt;
        }
      }
      
      setState({ output });
    } catch (error) {
      setState({ output: 'Error processing input: ' + (error instanceof Error ? error.message : 'Unknown error') });
    }
  };
  ...
  const toggleKeyManager = () => {
    setState({ showKeyManager: !state.showKeyManager });
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {state.output && !state.output.startsWith('Error:') && (
                <button
            ...
            <textarea
              value={state.output || ''}
              readOnly
              className={`w-full h-48 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-mono resize-none ${
